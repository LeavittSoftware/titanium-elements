import{S as e,b as t,c as o,A as r,_ as l,n as a,t as s,i,e as p}from"./BwDOgaCl.js";import{S as n}from"./DqlEvKiU.js";import"./BDkKqPD2.js";import"./DZ1f3jXa.js";import{T as c}from"./IroFLKwD.js";import{g as u}from"./OXmljVLM.js";import{D as d}from"./DTS7lhmZ.js";import{F as m}from"./BU8LG2ZX.js";import{a as g}from"./BS_tg9SS.js";import"./DKGGrv-j.js";import"./BPQf_WJy.js";import"./CGD201Wd.js";const h=new Map([["#lg.net.core.DataModel.Core.StatePeopleGroup",{icon:"location_searching",displayName:"State people group"}],["#lg.net.core.DataModel.Core.CityPeopleGroup",{icon:"share_location",displayName:"City people group"}],["#lg.net.core.DataModel.Core.JobRolePeopleGroup",{icon:"light_mode",displayName:"Job role people group"}],["#lg.net.core.DataModel.Core.CompanyPeopleGroup",{icon:"supervised_user_circle",displayName:"Company people group"}],["#lg.net.core.DataModel.Core.DepartmentPeopleGroup",{icon:"monetization_on",displayName:"Department people group"}],["#lg.net.core.DataModel.Core.BuildingPeopleGroup",{icon:"location_city",displayName:"Building people group"}],["#lg.net.core.DataModel.Core.CustomPeopleGroup",{icon:"tune",displayName:"Custom people group"}],["#lg.net.core.DataModel.Core.CompanyGroupPeopleGroup",{icon:"groups",displayName:"Company grouping people group"}],["#lg.net.core.DataModel.Core.StateManagerPeopleGroup",{icon:"deployed_code_account",displayName:"State manager people group"}],["#lg.net.core.DataModel.Core.HomeStatePeopleGroup",{icon:"other_houses",displayName:"Home state people group"}],["#lg.net.core.DataModel.Core.TypesOfInsuranceSoldPeopleGroup",{icon:"real_estate_agent",displayName:"Types of insurance sold people group"}],["#lg.net.core.DataModel.Core.TOISManagersPeopleGroup",{icon:"engineering",displayName:"Types of insurance sold managers people group"}]]);let v=class extends c{#e="Person or group";get label(){return this.#e}set label(e){this.#e=e}#t="Search for a person or group";get placeholder(){return this.#t}set placeholder(e){this.#t=e}#o="People";get peopleApiControllerName(){return this.#o}set peopleApiControllerName(e){this.#o=e}#r="PeopleGroups";get groupApiControllerName(){return this.#r}set groupApiControllerName(e){this.#r=e}#l="Name";get pathToSelectedText(){return this.#l}set pathToSelectedText(e){this.#l=e}#a="off";get autocomplete(){return this.#a}set autocomplete(e){this.#a=e}#s=!1;get spellcheck(){return this.#s}set spellcheck(e){this.#s=e}#i;get apiService(){return this.#i}set apiService(e){this.#i=e}#p=new d(e=>this.#n(e));#c=new AbortController;searchTermToOData(e){const t=u(e);return t.length<1?null:t.map(e=>`contains(tolower(Name), '${e.toLowerCase()}')`).join(" and ")}async#n(e){if(!e)return;this.#c.abort(),this.#c=new AbortController;const t=Promise.all([this.#u(e),this.#d(e)]);this.loadWhile(t);const o=await t,r=[...o[0]?.entities??[],...o[1]?.entities??[]],l=(o[0]?.odataCount??0)+(o[1]?.odataCount??0),a=new m(r,{includeScore:!0,ignoreLocation:!0,shouldSort:!0,keys:["Name"]}).search(e).slice(0,20);console.log(a),this.showSuggestions(a.map(e=>e.item)??[],l??0)}async#u(t){if(!t)return null;try{const e=["top=100","count=true","compute=FullName as Name","select=Name,Id,CompanyName,ProfilePictureCdnFileName"],o=u(t);if(o.length>0){const t=o.map(e=>`contains(tolower(Name), '${e.toLowerCase()}')`).join(" and ");e.push(`filter=${t}`)}const r=await(this.apiService?.getAsync(`${this.peopleApiControllerName}?${e.join("&")}`,{abortController:this.#c}));return r?.entities.forEach(e=>e.type="Person"),r}catch(t){t?.message?.includes("Abort error")||this.dispatchEvent(new e(t))}return null}async#d(t){if(!t)return null;const o=["not IsExpired"];try{const e=["top=100","count=true","select=Name,Id,Description","expand=EffectiveMembers(top=0;count=true)"],r=u(t);r.length>0&&o.push(r.map(e=>`contains(tolower(Name), '${e.toLowerCase()}')`).join(" and ")),e.push(`filter=${o.join(" and ")}`);const l=await(this.apiService?.getAsync(`${this.groupApiControllerName}?${e.join("&")}`,{abortController:this.#c}));return l?.entities.forEach(e=>e.type="PeopleGroup"),l}catch(t){t?.message?.includes("Abort error")||this.dispatchEvent(new e(t))}return null}static{this.styles=[...c.styles,t`
    ...
    `]}onInputChanged(e){this.#p.debounce(e)}renderSelectedLeadingInputSlot(e){return"Person"===e.type?o`<profile-picture slot="leading-icon" .fileName=${e?.ProfilePictureCdnFileName||null} shape="circle" size="24"></profile-picture>`:o`<md-icon group slot="leading-icon">${h.get(e["@odata.type"])?.icon??"task_alt"}</md-icon>`}renderSuggestion(e){return"Person"===e.type?o`<md-menu-item .item=${e} ?selected=${this.selected?.Id===e.Id}>
          ...
          <span slot="supporting-text">${e.CompanyName}</span>
            ...
            <md-icon group slot="start">${h.get(e["@odata.type"])?.icon??"task_alt"}</md-icon>
            <span slot="headline">${e.Name}</span>
            ...
            <span slot="supporting-text">${e.Description||(h.get(e["@odata.type"])?.displayName??"People group")}</span>
          </md-menu-item>`:o``}};l([a({type:String})],v.prototype,"label",null),l([a({type:String})],v.prototype,"placeholder",null),l([a({type:String})],v.prototype,"peopleApiControllerName",null),l([a({type:String})],v.prototype,"groupApiControllerName",null),l([a({type:String})],v.prototype,"pathToSelectedText",null),l([a({reflect:!0,type:String})],v.prototype,"autocomplete",null),l([a({reflect:!0,type:Boolean})],v.prototype,"spellcheck",null),l([a({attribute:!1})],v.prototype,"apiService",null),v=l([s("leavitt-person-group-select")],v);let S=class extends i{#m;get methodsSelect(){return this.#m}set methodsSelect(e){this.#m=e}static{this.styles=[n]}render(){return o`
        ...
        <main>
          ...
          <leavitt-app-width-limiter max-width="1000px">
            ...
            <div>
              <h1>Default</h1>
              <item-row>
                <leavitt-person-group-select label="default" .apiService=${g}></leavitt-person-group-select>
                <leavitt-person-group-select label="shaped" shaped .apiService=${g}></leavitt-person-group-select>
                ...
                <leavitt-person-group-select
                  ...
                  .apiService=${g}
                ></leavitt-person-group-select>
                <leavitt-person-group-select label="placeholder" placeholder="placeholder text" .apiService=${g}></leavitt-person-group-select>
                <leavitt-person-group-select
                  label="required"
                  required
            ...
            <div>
              <h1>Methods</h1>
              <item-row>
                <leavitt-person-group-select methods-demo required .apiService=${g}></leavitt-person-group-select>
              </item-row>
              <section buttons>
    ...
    `}};l([p("leavitt-person-group-select[methods-demo]")],S.prototype,"methodsSelect",null),S=l([s("leavitt-person-group-select-demo")],S);export{S as LeavittPersonGroupSelectDemo};
