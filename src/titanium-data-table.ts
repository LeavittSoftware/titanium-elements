...
export class TitaniumDataTable extends LitElement {
  ...
  private _handleItemSelectionChange(e: CustomEvent<{item: any, isSelected: boolean}>) {
    ...
    if (e.detail.isSelected) {
      if (this.singleSelect) {
        this._getTableItems().filter(o => o.item !== e.detail.item).forEach(o => o.deselected())
      }
  ...
  private _getTableItems(): Array<TitaniumDataTableItem> {
    return this.tableContanier[0].querySelector('slot[name="items"]').assignedNodes().filter(o => typeof o.select === 'function' || typeof o.deselect === 'function');
  }
...
<table-container>
  ...
  <no-results-indicator ?hidden="${this.isLoading || this.items.length > 0}"><svg viewBox="0 0 24 24">
      <path fill="none" d="M0 0h24v24H0V0z" />
      <path
        d="M11 7h2v2h-2zm0 4h2v6h-2zm1-9C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z" />
    </svg>
    ${this.searchTerm === '' || typeof this.searchTerm === 'undefined' || this.searchTerm === null ? 'No results' : `Your search of '${this.searchTerm}' did not match any results`}</no-results-indicator>
...
<table-controls ?hidden="${this.isLoading}">
  <table-control>
    ...
    <take-buttons>
      <titanium-svg-button path="M12 5.83L15.17 9l1.41-1.41L12 3 7.41 7.59 8.83 9 12 5.83zm0 12.34L8.83 15l-1.41 1.41L12 21l4.59-4.59L15.17 15 12 18.17z" @click="${this._handleChangeTake}"></titanium-svg-button>
    </take-buttons>
  </table-control>
  <div mobile-space></div>
  <table-control>
    <pagination-text>${this._getPageStats(this.page, this.count)}</pagination-text>
    <titanium-svg-button path="M15.41 7.41L14 6l-6 6 6 6 1.41-1.41L10.83 12z" @click="${this._handleLastPageClick}" ?disabled="${this.page === 0 || !this.count}"></titanium-svg-button>
    <titanium-svg-button path="M8.59,16.58L13.17,12L8.59,7.41L10,6L16,12L10,18L8.59,16.58Z" @click="${this._handleNextPageClick}" ?disabled="${(!this.count || (this.page + 1) * this.take >= this.count)}"></titanium-svg-button>
  </table-control>
</table-controls>`;
  }
}